  ...
  return (
    <CTAStyleWrapper>
      <div className="container">
        <SectionTitle
          className="CleverMeta_title_section text-center"
          title="Join Our community &amp; get Early access"
        ></SectionTitle>
        <div className="title-emo">🤙</div>
        <div className="CleverMeta_v1_cta_content">
            ...
            <ul>
              {data?.map((item, i) => (
                <li key={i}>
                  <a href={item.url}>
                    {item.thumb ? (
                      <img src={item.thumb} alt="CleverMeta nft profiles" />
                    ) : (
                      item.icon
                    )}
                  </a>
            ...
            <div className="footer_stars">
              {particleShapes?.map((shape, i) => (
                <span key={i} className={`star_${i + 1}`}>
                  <img src={shape} alt="CleverMeta nft background particle" />
                </span>
              ))}
            </div>
          </div>
